...
def complete_rotation(old_path: str, temp_path: str) -> str:
    """
    Call this AFTER the Patreon post has been verified to show the new link.

    Retires the previously-active folder (disables its export, moves it to
    the Rubbish Bin, kicks off a background purge) and finalizes temp_path
    as the new active folder (renaming it to FOLDER_FIXED_NAME if that's
    configured). Returns the final path of the active folder.
    """
    print(f"Patreon update verified - retiring old folder: {old_path}", flush=True)
    _disable_export_quiet(old_path)
    _delete_folder(old_path)
    _empty_trash_background()
    return finalize_active_folder(temp_path)


def rollback_rotation(temp_path: str) -> None:
    """
    Call this if the Patreon update failed or could not be verified.

    Removes the newly-created temp folder and its export, leaving the
    original folder (old_path) as the sole, still-exported, active folder
    - exactly as things were before rotate_mega_link() ran. old_path is
    deliberately left untouched here: its export was never disabled.
    """
    print(f"Rolling back - removing temp folder: {temp_path}", flush=True)
    _disable_export_quiet(temp_path)
