...
"""
Swaps the MEGA link inside a Patreon post's link block using Playwright.
No external stealth package – uses launch flags + init script to mask automation.
"""
...
def update_patreon_link(new_link: str, screenshot_dir: str = "artifacts") -> None:
    ...
    with sync_playwright() as playwright:
        ...
        # ---- Manual stealth via init script (no external package) ----
        ...
        try:
            ...
            context.close()
            browser.close()  Link block is added directly - so a first run, or a post someone already
